...
public class FacturaClientesController {
    ...
    @FXML private TableColumn<Cliente,String> colClienteNombre,colClienteNif,colClienteCorreo,colClienteTelefono;
    @FXML private TableColumn<Cliente,String> colClienteDireccion,colClienteCiudad,colClienteCp;
    ...
    @FXML private TableColumn<Proveedor,String> colProveedorNombre,colProveedorNif,colProveedorCorreo,colProveedorTelefono;
    @FXML private TableColumn<Proveedor,String> colProveedorDireccion,colProveedorCiudad,colProveedorCp;
    ...
    @FXML private TableColumn<Producto,String> colProductoCodigo,colProductoDescripcion,colProductoReferencia,colProductoIva,
    ...
    @FXML private void handleAñadir() {
    ...
    @FXML private void handleConsultar() {
        String tabName = MenuCliente.getSelectionModel().getSelectedItem().getText();
        if ("Cliente".equals(tabName)) refrescarClientes();
        else if ("Proveedor".equals(tabName)) refrescarProveedores();
        else if ("Producto".equals(tabName)) refrescarProductos();
        else if ("Facturas".equals(tabName)) refrescarFacturas();
    }
    ...
    private boolean validarNumero(TextField tf,String nombreCampo){
        ...
        catch(Exception e){mostrarError("El campo '"+nombreCampo+"' debe ser numérico.");return false;}
    }
    private void mostrarError(String msg){
        Alert a=new Alert(AlertType.ERROR);a.setTitle("Error");a.setHeaderText(null);a.setContentText(msg);a.showAndWait();
    }
    private void mostrarInfo(String msg){
        Alert a=new Alert(AlertType.INFORMATION);a.setTitle("Información");a.setHeaderText(null);a.setContentText(msg);a.showAndWait();
    }
    private boolean confirmar(String msg){
        Alert a=new Alert(AlertType.CONFIRMATION);a.setTitle("Confirmación");a.setHeaderText(null);a.setContentText(msg);
    ...
    */
    ...
    private void añadirCliente() {
        ...
        if (!validarCampoObligatorio(txtClienteNombre, "Nombre")) return;
        if (!validarCampoObligatorio(txtClienteNif, "NIF")) return;
    ...
    private void modificarCliente(){
        ...
        if(sel==null){mostrarError("Selecciona un cliente de la tabla.");return;}
    ...
    private void eliminarCliente(){
        Cliente sel=tablaClientes.getSelectionModel().getSelectedItem();
        if(sel==null){mostrarError("Selecciona un cliente de la tabla.");return;}
        if(!confirmar("¿Eliminar el cliente seleccionado?"))return;
        try{
    ...
    private void limpiarCliente(){
        ...
        txtClienteDireccion.clear();txtClienteCp.clear();txtClienteCiudad.clear();txtClienteProvincia.clear();txtClientePais.clear();
    }
    ...
    private void añadirProveedor() {
        ...
        if (!validarCampoObligatorio(txtProveedorNombre, "Nombre")) return;
        if (!validarCampoObligatorio(txtProveedorNif, "NIF")) return;
    ...
    private void limpiarProveedor(){
        ...
        txtProveedorDireccion.clear();txtProveedorCp.clear();txtProveedorCiudad.clear();txtProveedorProvincia.clear();txtProveedorPais.clear();
    }

    // CRUD Productos
    private void añadirProducto(){
        if(empresa==null){mostrarError("No hay empresa activa.");return;}
        if(!validarCampoObligatorio(txtProductoCodigo,"Código"))return;
        if(!validarCampoObligatorio(txtProductoDescripcion,"Descripción"))return;
        if(!validarNumero(txtProductoPrecioCoste,"Precio Coste"))return;
        if(!validarNumero(txtProductoPrecioVenta,"Precio Venta"))return;
        if(!validarNumero(txtProductoStock,"Stock"))return;
    ...
    private void modificarProducto(){
        ...
        String desc=safe(txtProductoDescripcion); if(!desc.isEmpty()) sel.setDescripcion(desc);
        String ref=safe(txtProductoReferencia);   if(!ref.isEmpty())  sel.setReferenciaProveedor(ref);
        String ivaTxt=safe(txtProductoIva);       if(!ivaTxt.isEmpty()) sel.setTipoIVAId(parseIntSafe(txtProductoIva,sel.getTipoIVAId()));
        String coste=safe(txtProductoPrecioCoste);if(!coste.isEmpty()) sel.setPrecioCoste(parseDoubleSafe(txtProductoPrecioCoste,sel.getPrecioCoste()));
        String venta=safe(txtProductoPrecioVenta);if(!venta.isEmpty()) sel.setPrecioVenta(parseDoubleSafe(txtProductoPrecioVenta,sel.getPrecioVenta()));
        String stock=safe(txtProductoStock);      if(!stock.isEmpty()) sel.setStock(parseDoubleSafe(txtProductoStock,sel.getStock()));
        try{
    ...
    private void limpiarProducto(){
        ...
        txtProductoIva.clear();txtProductoPrecioCoste.clear();txtProductoPrecioVenta.clear();txtProductoStock.clear();
    }

    // CRUD Facturas
    private void añadirFactura(){
        if(empresa==null){mostrarError("No hay empresa activa.");return;}
        if(comboFacturaTipo.getValue()==null){mostrarError("Selecciona el tipo de factura.");return;}
        if(!validarCampoObligatorio(txtFacturaNumero,"Número de factura"))return;
        if(!validarCampoObligatorio(txtFacturaFecha,"Fecha (yyyy-MM-dd)"))return;
        ...
        try{
            Date fecha=Date.valueOf(txtFacturaFecha.getText().trim()); // validación formato
            ...
            mostrarError("Error al añadir factura: "+e.getMessage());
        }
    }
    private void modificarFactura(){
        Factura sel=tablaFacturas.getSelectionModel().getSelectedItem();
        if(sel==null){mostrarError("Selecciona una factura de la tabla.");return;}
        if(comboFacturaTipo.getValue()!=null) sel.setTipo(mapTipoFactura(comboFacturaTipo.getValue()));
        String numero=safe(txtFacturaNumero); if(!numero.isEmpty()) sel.setNumero(numero);
        String fechaTxt=safe(txtFacturaFecha);
        if(!fechaTxt.isEmpty()){
            try{ sel.setFechaEmision(Date.valueOf(fechaTxt)); }
            catch(IllegalArgumentException fe){ mostrarError("Formato de fecha inválido (yyyy-MM-dd)."); return; }
        }
        String concepto=safe(txtFacturaConcepto); if(!concepto.isEmpty()) sel.setConcepto(concepto);
        String base=safe(txtFacturaBase); if(!base.isEmpty()) sel.setBaseImponible(parseDoubleSafe(txtFacturaBase,sel.getBaseImponible()));
        String iva=safe(txtFacturaIvaTotal); if(!iva.isEmpty()) sel.setIvaTotal(parseDoubleSafe(txtFacturaIvaTotal,sel.getIvaTotal()));
        String total=safe(txtFacturaTotal); if(!total.isEmpty()) sel.setTotalFactura(parseDoubleSafe(txtFacturaTotal,sel.getTotalFactura()));
        if(comboFacturaEstado.getValue()!=null) sel.setEstado(comboFacturaEstado.getValue());
        String obs=safe(txtFacturaObservaciones); if(!obs.isEmpty()) sel.setObservaciones(obs);
    ...
    private void eliminarFactura(){
        Factura sel=tablaFacturas.getSelectionModel().getSelectedItem();
        if(sel==null){mostrarError("Selecciona una factura de la tabla.");return;}
        if(!confirmar("¿Eliminar la factura seleccionada?"))return;
        try{
    ...
    private void limpiarFactura(){
        txtFacturaNumero.clear();txtFacturaFecha.clear();txtFacturaConcepto.clear();
        txtFacturaBase.clear();txtFacturaIvaTotal.clear();txtFacturaTotal.clear();txtFacturaObservaciones.clear();
    ...
    private double parseDoubleSafe(TextField tf,double def){ try{ return Double.parseDouble(tf.getText().trim()); }catch(Exception e){ return def; } }
    private String formatDouble(Double d){ return d==null?"":String.format("%.2f",d); }
    private char mapTipoFactura(String tipoUi){ return "Venta".equalsIgnoreCase(tipoUi)?'V':'C'; }
    ...
    private String obtenerDireccionCampo(Long entidadId,String campo){
        ...
        for(Direccion d: todas){
            if(entidadId.equals(d.getEntidadId())){
                ...
                if("ciudad".equals(campo)) return safeStr(d.getCiudad());
                if("cp".equals(campo)) return safeStr(d.getCp());
                return "";
            }
        }
        return "";
    ...
    private void añadirDireccionSiRellenada(Long entidadId,
        ...
        if(entidadId==null || entidadId<=0) return;
        String etiqueta= comboEtiqueta!=null ? comboEtiqueta.getValue() : null;
        String direccion=safe(txtDir);
        if(etiqueta==null || etiqueta.isEmpty() || direccion.isEmpty()) return;
